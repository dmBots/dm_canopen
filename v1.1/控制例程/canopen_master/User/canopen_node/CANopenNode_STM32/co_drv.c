...
CO_CANsetConfigurationMode(void* CANptr) {
    ...
    if (CANptr != NULL) {
        ...
        HAL_FDCAN_Stop(((canopen_node_t*)CANptr)->CANHandle);
#else
...
#ifdef CO_STM32_FDCAN_Driver
        if (HAL_FDCAN_Start(((canopen_node_t*)CANmodule->CANptr)->CANHandle) == HAL_OK)
#else
...
CO_CANmodule_init(CO_CANmodule_t* CANmodule, void* CANptr, CO_CANrx_t rxArray[], uint16_t rxSize, CO_CANtx_t txArray[],
    ...
    /***************************************/
    ((canopen_node_t*)CANptr)->HWInitFunction();
...
#ifdef CO_STM32_FDCAN_Driver
    if (HAL_FDCAN_ConfigGlobalFilter(((canopen_node_t*)CANptr)->CANHandle, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_REJECT,
...
#ifdef CO_STM32_FDCAN_Driver
    if (HAL_FDCAN_ActivateNotification(((canopen_node_t*)CANptr)->CANHandle,
...
#ifdef CO_STM32_FDCAN_Driver
        HAL_FDCAN_Stop(((canopen_node_t*)CANmodule->CANptr)->CANHandle);
...
prv_send_can_message(CO_CANmodule_t* CANmodule, CO_CANtx_t* buffer) {
    ...
    if (HAL_FDCAN_GetTxFifoFreeLevel(((canopen_node_t*)CANmodule->CANptr)->CANHandle) > 0) {
        /*
        ...
        success =
            HAL_FDCAN_AddMessageToTxFifoQ(((canopen_node_t*)CANmodule->CANptr)->CANHandle, &tx_hdr, buffer->data)
            == HAL_OK;
    }
#else
...
CO_CANmodule_process(CO_CANmodule_t* CANmodule) {
    ...
    err = ((FDCAN_HandleTypeDef*)((canopen_node_t*)CANmodule->CANptr)->CANHandle)->Instance->PSR
